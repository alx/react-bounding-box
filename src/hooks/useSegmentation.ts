...
import type { UseSegmentationConfig, UseSegmentationReturn, SegmentationMask } from '@/types';
 ...
 */
export const useSegmentation = (config: UseSegmentationConfig): UseSegmentationReturn => {
  const {
    segmentationData,
    segmentationMasks,
    colors,
  ...
  const [segmentColors, setSegmentColors] = useState(new Map<number, [number, number, number]>());
   ...
   */
  const generateSegmentColor = useCallback((classIndex: number): [number, number, number] => {
  ...
  const renderSegmentation = useCallback((
    ...
    const targetCanvas = separateCanvas ? segmentationCanvasRef.current : targetCanvasRef.current;
