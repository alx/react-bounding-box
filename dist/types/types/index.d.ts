...
export type BoundingBox = number[] | BoundingBoxCoord | BoundingBoxMinMax | BoundingBoxWithLabel;
...
export type CoordinateTransform = (coord: number[] | BoundingBoxCoord | BoundingBoxMinMax) => [number, number, number, number];
export type ColorGenerator = (classIndex: number) => [number, number, number];
export type CanvasDrawFunction = (ctx: CanvasRenderingContext2D, box: BoundingBox, color: string, lineWidth: number) => void;
export type LabelDrawFunction = (ctx: CanvasRenderingContext2D, box: BoundingBoxWithLabel) => void;
